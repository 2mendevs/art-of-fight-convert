...
const IconChat = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><line x1="9" y1="10" x2="9" y2="10" strokeWidth="2.5"/><line x1="12" y1="10" x2="12" y2="10" strokeWidth="2.5"/><line x1="15" y1="10" x2="15" y2="10" strokeWidth="2.5"/>
  </svg>
);
const IconLeaf = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    <path d="M12 20 C12 20 4 16 4 9 C4 9 8 6 12 10"/><path d="M12 10 C12 10 8 5 12 2 C12 2 16 5 12 10"/><path d="M12 20 C12 20 20 16 20 9 C20 9 16 6 12 10"/><path d="M12 20 C12 20 12 14 12 10"/><path d="M8 10 C9 11 11 12 12 14" strokeWidth="1" opacity="0.7"/><path d="M16 10 C15 11 13 12 12 14" strokeWidth="1" opacity="0.7"/>
  </svg>
);
const IconChart = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    <line x1="3" y1="20" x2="21" y2="20"/><polyline points="5,20 5,13 9,13 9,20"/><polyline points="10,20 10,9 14,9 14,20"/><polyline points="15,20 15,5 19,5 19,20"/>
  </svg>
);
const IconGlobe = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"/>
  </svg>
);
const IconTarget = () => (
...
const IconTrophy = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="8 21 12 17 16 21"/><line x1="12" y1="17" x2="12" y2="11"/><path d="M7 4H4v3a5 5 0 0 0 5 5h6a5 5 0 0 0 5-5V4h-3"/><rect x="7" y="2" width="10" height="4" rx="1"/>
  </svg>
);
const IconShield = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#07b4ba" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
const faqItems = [
  {
    q: "Do I need prior MMA or martial arts experience?",
    a: "No experience needed at all. Our 1-on-1 coaching is designed to meet you exactly where you are — whether you've never trained a day in your life or you're returning after a break. The program is built from the ground up for your current level."
  },
  {
    q: "How long are the coaching sessions?",
    a: "Sessions typically run 60 to 90 minutes depending on the training phase. During competition prep phases, sessions may extend. Your coach will always communicate the schedule in advance so you can plan accordingly."
  },
  {
    q: "What disciplines are covered in the program?",
    a: "The program covers all core MMA disciplines — striking (boxing, Muay Thai), wrestling, Brazilian Jiu-Jitsu, clinch work, and MMA-specific combinations. The balance between disciplines is personalised based on your goals."
  },
  {
    q: "How quickly will I see results?",
    a: "Most athletes notice measurable improvement in technique and conditioning within the first 3 to 4 weeks. Significant transformation — in fight IQ, timing, and physical conditioning — typically shows within 8 to 12 weeks of consistent training."
  },
  {
    q: "What if I need to pause or reschedule a session?",
    a: "Life happens. You can reschedule any session with at least 24 hours' notice at no penalty. We work around your schedule — not the other way around. Our goal is to keep you progressing, not to penalise unavoidable situations."
  },
];
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* HERO */
  .cp-hero { position: relative; min-height: 100vh; display: flex; flex-direction: column; overflow: hidden; }
  .cp-hero-bg { position: absolute; inset: 0; z-index: 0; }
  .cp-hero-overlay { position: absolute; inset: 0; z-index: 1; background: linear-gradient(to right, #0a0a0a 42%, rgba(10,10,10,0.72) 65%, transparent 100%); }
  ...
  .cp-trust-item span { font-family: 'Bebas Neue', sans-serif; color: #000; font-size: clamp(14px,2vw,18px); letter-spacing: 2px; white-space: nowrap; }
  ...
  .cp-method-item { display: flex; align-items: flex-start; gap: 14px; margin-bottom: 18px; padding: 14px 16px; border-radius: 10px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); }
  ...
  .cp-what-card { width: 170px; padding: 22px 12px; border-radius: 14px; background: #1a1a1a; border: 2px solid #07b4ba; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 10px; }
  .cp-what-card-icon { width: 50px; height: 50px; border-radius: 10px; border: 1.5px solid rgba(7,180,186,0.35); display: flex; align-items: center; justify-content: center; background: rgba(7,180,186,0.06); }
  ...
  /* OUR PROMISE */
  .cp-promise { max-width: 620px; margin: 36px auto 0; text-align: center; padding: 0 20px; }
  .cp-promise-quote-mark { font-family: 'Bebas Neue', sans-serif; font-size: 72px; line-height: 0.6; color: #07b4ba; opacity: 0.5; display: block; margin-bottom: 8px; }
      ...
      {selectedDay && (
        <div>
          ...
          <div className="cp-time-slots">
            {timeSlots.map(t => (
              <button key={t} className={`cp-time-slot${selectedTime === t ? " selected" : ""}`} onClick={() => setSelectedTime(t)}>{t}</button>
            ))}
          </div>
        </div>
      )}
      {selectedDay && selectedTime && (
        <button className="cp-submit" onClick={() => onConfirm(`${months[month]} ${selectedDay}, ${year}`, selectedTime)}>Confirm Booking</button>
      )}
    </div>
  );
}
      ...
      {faqItems.map((item, i) => (
        <div key={i} className="cp-faq-item">
          <button className="cp-faq-q" onClick={() => setOpen(open === i ? null : i)}>
            <span>{item.q}</span>
            <span className={`cp-faq-chevron${open === i ? " open" : ""}`}>
              <ChevronDown />
            </span>
          </button>
...
export default function CoachingPage() {
  ...
  const handleLeadSubmit = () => { if (lead.name.trim() && lead.phone.trim()) setStage(2); };
  const handleBookingConfirm = (_date: string, _time: string) => { setStage(3); };
    ...
    <>
      ...
      <div className="cp">
        ...
        <section className="cp-hero">
          <div className="cp-hero-bg" style={{ background: `linear-gradient(to right, #0a0a0a 38%, rgba(10,10,10,0.65) 62%, transparent 100%), url("https://images.unsplash.com/photo-1549476464-37392f717541?w=1400&q=80") center right / cover no-repeat` }} />
            ...
            <Reveal>
              <div className="cp-pain-grid">
                <div className="cp-pain-media">
                  <img src="https://images.unsplash.com/photo-1549060279-7e168fcee0c2?w=900&q=80" alt="UFC Training" />
                </div>
                <div className="cp-pain-right">
                  ...
                  {painPoints.map((item, i) => (
                    <div className="cp-pain-item" key={i}><p>{item}</p></div>
                  ))}
                </div>
              </div>
            </Reveal>
        ...
        <div className="cp-method-bg">
          <div className="cp-section" style={{ padding: "70px 40px" }}>
            <Reveal>
              <div style={{ textAlign: "center", marginBottom: 48 }}>
                <p style={{ color: "#07b4ba", fontFamily: "'Barlow', sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 3, textTransform: "uppercase", marginBottom: 6 }}>The AOF Method</p>
                <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(36px,5vw,56px)", letterSpacing: 2, color: "#fff", lineHeight: 1 }}>A Proven System.</h2>
                <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(36px,5vw,56px)", letterSpacing: 2, color: "#07b4ba", lineHeight: 1 }}>Real Transformation.</h2>
                <p style={{ color: "rgba(255,255,255,0.38)", fontFamily: "'Barlow', sans-serif", fontSize: 11, marginTop: 10, letterSpacing: 3, textTransform: "uppercase" }}>Complete Support. Every Step.</p>
            ...
            <div className="cp-method-grid">
              ...
              <div className="cp-method-image">
                <img src="https://images.unsplash.com/photo-1605296867424-35fc25c9212a?w=900&q=80" alt="MMA Training" style={{ width: "100%", borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)", objectFit: "cover" }} />
              </div>
            </div>

            {/* WHAT YOU GET */}
            <Reveal style={{ marginTop: 52 }}>
              <h3 style={{ textAlign: "center", fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, letterSpacing: 2, color: "#fff", marginBottom: 20 }}>What You Get</h3>
            ...
            <Reveal>
              <div className="cp-promise">
                ...
                <h4>Our Promise</h4>
                <p>Follow the system for 30 days. If you don't improve, we extend your coaching — free. We succeed when you succeed. That's not a slogan, it's our commitment.</p>
              </div>
            </Reveal>
          </div>
        </div>
        ...
        <div id="cp-testimonials" style={{ background: "#0d0d0d" }}>
        ...
        <div className="cp-apply-bg" ref={formRef}>
          <div className="cp-section">
            <div style={{ display: "flex", gap: 56, alignItems: "flex-start", flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 260 }}>
                <Reveal>
                  <p style={{ fontFamily: "'Barlow', sans-serif", color: "#07b4ba", fontWeight: 700, fontSize: 12, letterSpacing: 2.5, marginBottom: 12, textTransform: "uppercase" }}>Ready To Start?</p>
                  ...
                  {checklistItems.map((item, i) => (
                    <div className="cp-checklist-item" key={i}>
                      <span className="check">✓</span><p>{item}</p>
                    </div>
                  ))}
                  <div style={{ marginTop: 32 }}>
                    <p className="cp-wa-label">Any Queries?</p>
                    <button className="cp-wa-btn"><WhatsAppIcon />Contact Us on WhatsApp</button>
                  </div>
                </Reveal>
              </div>
